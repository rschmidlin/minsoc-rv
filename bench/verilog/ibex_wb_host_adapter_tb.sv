...
module ibex_wb_host_adapter_tb;
  ...
  // C17: fifo_empty in the middle of a burst (no non-sequential terminator)
  //
  ...
  task test_fifo_empty_midburst;
    ...
    integer waited;
    begin
      start_test("C17: fifo_empty in the middle of a burst (all addresses sequential)");
      stop_ack();
      ...
      req_valid = 1'b0;
      check(granted >= 3, "C17: need at least a 3-deep sequential run to exercise mid-burst empty");
      ...
      check(resp_valid == 1'b0,
            "C17: phantom resp_valid after FIFO emptied mid-burst");
      check(!wb_cyc, "C17: WB cycle must close after the burst drains");
      expect_counts(granted, granted);
      check(sb_addr[0] == 32'h0000_0800, "C17: first grant 0x800");
    end
  endtask
